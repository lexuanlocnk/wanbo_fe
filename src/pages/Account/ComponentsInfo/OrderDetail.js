  ...
  return (
    <Col className="container mb-4">
      <div className="p-1 my-4">
        <Col
          style={{ fontSize: 16 }}
          className="d-flex fw-bold justify-content-between"
        >
      ...
      <div className="p-1 my-4">
        ...
        >
          <div>
            ...
            <span style={{ color: "orange" }}>{orders.order_status}</span>{" "}
          </div>
          <div className="">
          ...
          <Row style={{ fontSize: 16 }} className="justify-content-center">
            <Col md={6} className="d-md-block ">
              ...
              <div className="border p-3" style={{ borderRadius: 5 }}>
                {orders.d_name}
                <div>Địa chỉ: {orders.d_address}</div>
                <div>Số điện thoại: {orders.d_phone}</div>
              </div>
            </Col>
            <Col md={3} className="d-md-block ">
              THANH TOÁN
              <div className="border p-3" style={{ borderRadius: 5 }}>
                {orders.payment_method?.title
                  ? orders.payment_method?.title
                  : "..."}{" "}
                <br />
                <br />
                {orders.shipping_method?.title
                  ? orders.shipping_method?.title
                  : "..."}
              </div>
            </Col>
            <Col md={3} className="d-md-block ">
              GHI CHÚ
              <div className="border p-3" style={{ borderRadius: 5 }}>
                <div>{orders.note ? orders.note : "Không có ghi chú "}</div>
              </div>
            </Col>
          </Row>
          ...
          <Card className="mt-4" style={{ border: "none" }}>
            <Card.Body>
              <Card.Title
                ...
                className="d-flex text-end px-4"
              >
                <div style={{ width: "40%" }} className="d-flex text-start">
                  Đơn hàng{" "}
                </div>
              ...
              <ul className="list-group">
                {orders.orderDetail?.map((item) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 14, width: "30%" }}>
                      {item.ProductName}
                    </div>
                    ...
                    >
                      {item.subtotal
                        ? `${(item.subtotal * 1).toLocaleString("vi-VN")} đ`
                        : ""}
                      {/* {item.subtotal} */}
                    </div>
                  </li>
              ...
              <Row>
                ...
                >
                  {orders.total_cart
                    ? `${orders.total_cart.toLocaleString("vi-VN")} đ`
                    : ""}
                </Col>
              </Row>
              ...
              <Row>
                ...
                <Col
                  ...
                  className="text-end"
                >
                  {orders.CouponDiscout
                    ? `${orders.CouponDiscout.toLocaleString("vi-VN")} đ`
                    : "0"}
                </Col>
              </Row>
              ...
              <div className="mt-3 d-flex justify-content-between">
                ...
                <h5 style={{ color: "#0d6efd" }}>
                  {orders.total_price
                    ? `${orders.total_price.toLocaleString("vi-VN")} đ`
                    : ""}
                </h5>
              </div>
            </Card.Body>
