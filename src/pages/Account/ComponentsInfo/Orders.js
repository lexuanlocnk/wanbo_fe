...
const Orders = () => {
  ...
  return (
    <Col>
      ...
      <div
        className="border p-1 d-none d-md-block"
          ...
          orders.map((order) => (
            ...
            >
              <Row className="align-items-center justify-content-center">
                <Col md={2}>
                  <Link to={`/information/${order.order_id}`}>
                    <strong>{order.order_code}</strong>
                  </Link>
                </Col>
                <Col md={2}>
                  {new Date(order.date_order * 1000).toLocaleDateString(
                    "vi-VN"
                  )}
                </Col>
                <Col md={2}>{order.d_address}</Col>
                <Col md={2}>{order.total_price.toLocaleString("vi-VN")} đ</Col>
                <Col md={2}>
          ...
          orders.map((order) => (
            ...
            >
              <Row>
                ...
                <Col md={2}>Địa chỉ: {order.d_address}</Col>
                <Col md={2}>
                  Giá trị đơn hàng: {order.total_price.toLocaleString("vi-VN")}{" "}
                  đ
                </Col>
                <Col md={2}>
                  TT thanh toán:{" "}
